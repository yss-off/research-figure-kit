...
- 新增 `drawio-poster` 海报工作流，明确主流程与备注的层级、品牌原图保留、讨论/修改边界和局部修订策略；
- 增加目标显示宽度的字号审查与实际字体宽度换行工具，区分结构通过、视觉判断和用户验收；
- 增加海报路由与行为回归，记录本次迭代经验及上游渲染器缺陷证据；
- 论文 overlay 更新为 0.1.3：区分共有机制与实际改动，将解释样例关联到生成阶段，并减少图内长篇说明；
- 保持固定上游 drawio 与数据图运行时代码不变；底层参考线和装饰网格问题仅记录，未宣称修复。
